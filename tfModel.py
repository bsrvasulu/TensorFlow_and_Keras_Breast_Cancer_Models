...
def random_minibatches(X, Y, minimatch_size = 32):
    m = X.shape[1]
    mini_batches = []
...
def initiliaze_weights_bias(network_shape):
    parameters = {}
...
def initiliaze_weights_bias2(network_shape):
    """
...
def compute_cost2(Z, Y):
    logits = tf.transpose(Z)
    labels = tf.transpose(Y)    
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = logits, labels = labels))    
    return cost

def compute_cost(finalZ, Y):
...
def model(X_train, Y_train, X_test, Y_test, network_shape, learning_rate = 0.0001, num_epochs = 200, minibatch_size = 32, print_cost = True, save_weights = True):
    ...
    parameters = initiliaze_weights_bias(network_shape)
    
    #farward propagation 
    Z_final = farward_propagation(X, parameters)
...
def restore_model(X_test, Y_test, network_shape):
